...
class Session extends ComponentBase
{
    ...
    {
        return [
            'name'        => 'crytofy.user::lang.session.session',
            'description' => 'crytofy.user::lang.session.session_desc'
        ];
    }
    ...
    public function defineProperties()
    {
        return [
            'security' => [
                'title'       => 'crytofy.user::lang.session.security_title',
                'description' => 'crytofy.user::lang.session.security_desc',
                'type'        => 'dropdown',
                'default'     => 'all',
                'options'     => [
                    'all'   => 'crytofy.user::lang.session.all',
                    'user'  => 'crytofy.user::lang.session.users',
                    'guest' => 'crytofy.user::lang.session.guests'
                ]
            ],
            'allowedUserGroups' => [
                'title'       => 'crytofy.user::lang.session.allowed_groups_title',
                'description' => 'crytofy.user::lang.session.allowed_groups_description',
                'placeholder' => '*',
                'type'        => 'set',
                'default'     => []
            ],
            'redirect' => [
                'title'       => 'crytofy.user::lang.session.redirect_title',
                'description' => 'crytofy.user::lang.session.redirect_desc',
                'type'        => 'dropdown',
                'default'     => ''
            ]
        ];
     ...
     * @return Crytofy\User\Models\User
     */
    ...
    public function onLogout()
    {
        ...
        if ($user) {
            Event::fire('crytofy.user.logout', [$user]);
        }

        $url = post('redirect', Request::fullUrl());

        Flash::success(Lang::get('crytofy.user::lang.session.logout'));
    ...
    public function onStopImpersonating()
    {
        ...
        Flash::success(Lang::get('crytofy.user::lang.session.stop_impersonate_success'));
